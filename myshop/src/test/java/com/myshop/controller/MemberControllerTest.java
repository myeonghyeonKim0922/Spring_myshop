...
@SpringBootTest
@AutoConfigureMockMvc //실제 객체와 비슷하지만 테스트에 필요한 기능만 제공하는 가짜 객체를 만든다. -> 웹 브라우저에서 요청을 하는 것처럼 작성 가능
@Transactional
...
class MemberControllerTest {
	...
	public Member createMember(String email, String password) {
		...
		Member member2 = Member.createMember(member, passwordEncorder);
	...
	public void loginFailTest() throws Exception {
		 String email = "test@email.com";
	        ...
	        mockMvc.perform(formLogin().userParameter("email")
	                ...
	                .andExpect(SecurityMockMvcResultMatchers.unauthenticated()); //로그인이 실패하 테스트 코드를 통과시킨다.
	}
}
